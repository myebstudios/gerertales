...
const AppNavigation: React.FC<AppNavigationProps> = ({ currentState, onNavigate, userProfile }) => {
    ...
    <div className="w-16 h-full bg-dark-surface border-r border-dark-border flex flex-col items-center py-6 gap-6 z-20">
      ...
      <button
        ...
        className={`p-3 rounded-xl transition-all duration-300 group relative
          ${currentState === AppState.LIBRARY ? 'bg-zinc-800 text-cobalt' : 'text-zinc-500 hover:text-text-main hover:bg-zinc-800/50'}`}
        title="Library"
      >
      ...
      {/* Create / Writing */}
      <button
        onClick={() => onNavigate(AppState.ONBOARDING)}
        className={`p-3 rounded-xl transition-all duration-300 group relative
          ${currentState === AppState.ONBOARDING ? 'bg-zinc-800 text-cobalt' : 'text-zinc-500 hover:text-text-main hover:bg-zinc-800/50'}`}
        title="New Story"
      >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
      ...
      <button
        ...
        className={`p-3 rounded-xl transition-all duration-300 group relative
          ${currentState === AppState.SETTINGS ? 'bg-zinc-800 text-cobalt' : 'text-zinc-500 hover:text-text-main hover:bg-zinc-800/50'}`}
        title="Settings"
      >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
      ...
      <div className="mt-auto flex flex-col items-center gap-4 mb-6 relative">
        ...
        <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
            ...
            <button
                ...
                ${currentState === AppState.PROFILE ? 'border-cobalt' : 'border-transparent hover:border-zinc-500'}`}
