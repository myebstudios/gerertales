    ...
    >
        ...
        <div className="p-6 flex-1 flex flex-col relative">
            ...
            <div className="mt-auto flex items-center justify-between pt-6 border-t border-dark-border/50">
                {story.isPublic && story.ownerName && story.ownerId !== currentUserId ? (
                    <div className="flex items-center gap-2">
                        {story.ownerAvatar ? (
                            <img src={story.ownerAvatar} alt={story.ownerName} className="w-5 h-5 rounded-full object-cover border border-zinc-700" />
                        ) : (
                            <div className="w-5 h-5 rounded-full bg-gradient-to-br from-cobalt to-purple-600 flex items-center justify-center text-[9px] text-white font-bold border border-white/10 uppercase">
                                {story.ownerName.charAt(0)}
                            </div>
                        )}
                        <span className="text-[10px] text-zinc-400 font-medium truncate max-w-[100px]" title={story.ownerName}>
                            {story.ownerName}
                        </span>
                    </div>
                ) : (
