    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            if (story.coverImage) {
                try {
                    yPosition += 20;
                    // Fetch image to bypass CORS and ensure it's loaded
                    const response = await fetch(story.coverImage);
                    ...
                    doc.addImage(base64, 'PNG', (pageWidth - imgWidth) / 2, yPosition, imgWidth, imgHeight);
                } catch (e) {
            ...
            // Chapters
            for (const chapter of story.toc) {
                doc.addPage();
                yPosition = 30;
                ...
                // Chapter Banner
                if (chapter.bannerImage) {
                    try {
                        const response = await fetch(chapter.bannerImage);
                        const blob = await response.blob();
                        const base64 = await new Promise<string>((resolve) => {
                            const reader = new FileReader();
                            reader.onloadend = () => resolve(reader.result as string);
                        ...
                        console.warn(`Could not add banner for chapter ${chapter.chapter}`, e);
                    }
                }
                ...
                // Split text into paragraphs to handle spacing better
            ...
            for (let i = 1; i <= totalPages; i++) {
                ...
                doc.text(`Generated by GérerTales on ${new Date().toLocaleDateString()}`, pageWidth - margin, pageHeight - 10, { align: "right" });
            }
            ...
            notify("Export failed. Please check console.");
        } finally {
    ...
    return (
        <div className="flex flex-col h-full bg-dark-bg relative transition-all duration-500 overflow-hidden">
                        ...
                        {showExportMenu && (
                            <div className="absolute right-0 top-full mt-3 w-56 bg-dark-surface border border-dark-border rounded-2xl shadow-2xl py-2 z-50 animate-in fade-in zoom-in-95 duration-200">
                                ...
                                <button
                                    ...
                                    className="w-full text-left px-4 py-3 text-xs text-text-main hover:bg-zinc-800 flex items-center gap-2 transition-colors disabled:opacity-50"
                                >
            ...
            <div className="flex-1 overflow-y-auto relative no-scrollbar" onClick={() => { setShowExportMenu(false); setShowSettings(false); setShowChapterDropdown(false); }}>
                ...
                <div className={`px-4 sm:px-6 md:px-12 lg:px-24 xl:px-32 pb-24 md:pb-48 relative z-20 ${!chapter.bannerImage && !isBannerLoading ? 'pt-8 md:pt-16' : '-mt-8 md:-mt-16'}`}>
            ...
            {/* Floating Audio Bar - Responsive */}
            <div className={`absolute bottom-4 md:bottom-8 left-1/2 -translate-x-1/2 z-40 w-[calc(100%-2rem)] md:w-full max-w-2xl px-3 md:px-6 transition-all duration-700 ${focusMode ? 'translate-y-24 opacity-0' : 'translate-y-0 opacity-100'}`}>
                <div className="bg-dark-surface/90 backdrop-blur-2xl border border-white/5 rounded-2xl md:rounded-[2rem] p-2 md:p-3 px-3 md:px-6 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-6 group/audio">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                        <button
                            ...
                            className="w-11 h-11 md:w-12 md:h-12 rounded-full bg-cobalt text-white flex items-center justify-center hover:bg-blue-500 transition-all shadow-xl shadow-cobalt/20 active:scale-95 shrink-0"
                            aria-label={isPlaying ? "Pause narration" : "Play narration"}
                        >
                            {isLoadingAudio ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : isPlaying ? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 md:w-6 md:h-6"><path fillRule="evenodd" d="M6.75 5.25a.75.75 0 01.75-.75H9a.75.75 0 01.75.75v13.5a.75.75 0 01-.75.75H7.5a.75.75 0 01-.75-.75V5.25zm7.5 0A.75.75 0 0115 4.5h1.5a.75.75 0 01.75.75v13.5a.75.75 0 01-.75.75H15a.75.75 0 01-.75-.75V5.25z" clipRule="evenodd" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 md:w-6 md:h-6 ml-0.5"><path fillRule="evenodd" d="M4.5 5.653c0-.856.917-1.398 1.667-.986l11.54 6.348a1.125 1.125 0 010 1.971l-11.54 6.347a1.125 1.125 0 01-1.667-.985V5.653z" clipRule="evenodd" /></svg>}
                        </button>

                        <button
                            onClick={() => { stopAudio(); setPlaybackProgress(0); }}
                            className="w-11 h-11 md:w-10 md:h-10 rounded-full bg-zinc-800/50 text-zinc-500 flex items-center justify-center hover:bg-red-500/10 hover:text-red-500 transition-all shrink-0 active:scale-95"
                            title="Stop"
